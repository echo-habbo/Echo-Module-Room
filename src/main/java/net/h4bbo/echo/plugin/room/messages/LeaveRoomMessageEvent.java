...
public class LeaveRoomMessageEvent extends MessageEvent<RoomPlugin> {
    @Override
    public void handle(IPlayer player, IClientCodec msg) {
        player.getConnection().getMessageHandler().deregister(this.getPlugin(), LeaveRoomMessageEvent.class);

        if (!player.isInRoom())
            return;

        player.getRoomEntity().getRoom().getEntityManager().leave(player);
    }
